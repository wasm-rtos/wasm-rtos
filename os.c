...
#define OS_WASM_LIBRARY_DEFAULT_STACK_SIZE (32U * 1024U)
...
static const char OS_WASM_LIBRARY_EXPORT_NOT_FOUND_ERROR[] =
    "WASM library export not found";
static const char OS_WASM_LIBRARY_SIGNATURE_MISMATCH_ERROR[] =
    "WASM library function signature mismatch";
...
typedef struct OsWasmLibraryExport
{
    char* name;
    OsValueType* return_types;
    OsValueType* argument_types;
    uint32_t return_count;
    uint32_t argument_count;
    struct OsWasmLibraryExport* next;
} OsWasmLibraryExport;
...
struct OsWasmLibrary
{
    ...
    OsWasmLibraryExport* exports;
    uint32_t binding_count;
    struct OsWasmLibrary* previous;
    struct OsWasmLibrary* next;
};

typedef struct OsWasmLibraryInstance
{
    OsTaskHandle task;
    ...
    uint8_t source_acquired;
    uint8_t active;
    uint8_t listed;
    struct OsWasmLibraryInstance* previous;
    struct OsWasmLibraryInstance* next;
} OsWasmLibraryInstance;

typedef struct OsWasmLibraryImportBinding
{
    OsTaskHandle task;
    OsWasmLibraryHandle library;
    OsWasmLibraryExport* library_export;
    struct OsWasmLibraryImportBinding* next;
} OsWasmLibraryImportBinding;

typedef struct OsPendingWasmLibraryCall
{
    OsWasmLibraryImportBinding* binding;
    OsWasmLibraryInstance* instance;
    IM3Function function;
    uint64_t* values;
    const void** argument_pointers;
    const void** return_pointers;
    uint32_t return_offset;
    uint8_t start_suspended;
    uint8_t started;
} OsPendingWasmLibraryCall;
...
typedef struct OsState
{
    ...
    OsWasmLibraryInstance* wasm_library_instance_list;
    ...
    uint64_t next_wasm_library_use_sequence;
    uint64_t wasm_library_cache_limit_bytes;
    uint64_t wasm_library_resident_size_bytes;
    uint32_t task_count;
    ...
    uint8_t clock_wakeup_armed;
    uint8_t processing_time_events;
    uint32_t clock_armed_tick_ms;
...
struct OsTask
{
    ...
    uint32_t run_count;
    OsWasmLibraryImportBinding* wasm_library_imports;
    OsPendingWasmLibraryCall* pending_wasm_library_call;
...
static OsTimerHandle os_timer_allocate(void);
static void os_timer_free(OsTimerHandle timer);
static void os_timer_list_insert(OsTimerHandle timer);
static void os_timer_list_remove(OsTimerHandle timer);
static int os_timer_is_in_list(OsTimerHandle timer);
static void os_timer_free_all(void);
static void os_timer_delete_for_task(OsTaskHandle task);
static void os_process_timers(void);
...
static OsValueType os_wasm_library_value_type(M3ValueType type);
static uint8_t os_wasm_library_function_matches_export(
    IM3Function function,
    OsWasmLibraryExport* library_export
);
static int os_wasm_library_has_host_import_module(const char* module_name);
static void os_wasm_library_export_free_all(OsWasmLibraryExport* library_export);
static OsWasmLibraryExport* os_wasm_library_export_find(
    OsWasmLibraryHandle library,
    const char* export_name
);
static OsStatus os_wasm_library_validate_source(
    OsWasmLibraryHandle library
);
static void os_wasm_library_free(OsWasmLibraryHandle library);
static void os_wasm_library_free_all(void);
static int os_wasm_library_is_in_list(OsWasmLibraryHandle library);
static void os_wasm_library_list_insert(OsWasmLibraryHandle library);
static void os_wasm_library_list_remove(OsWasmLibraryHandle library);
static OsWasmLibraryInstance* os_wasm_library_instance_find(
    OsTaskHandle task,
    OsWasmLibraryHandle library
);
static void os_wasm_library_instance_insert(
    OsWasmLibraryInstance* instance
);
static void os_wasm_library_instance_remove(
    OsWasmLibraryInstance* instance
);
static uint64_t os_wasm_library_runtime_size(
    OsWasmLibraryInstance* instance
);
static void os_wasm_library_instance_refresh_size(
    OsWasmLibraryInstance* instance
);
static void os_wasm_library_instance_touch(
    OsWasmLibraryInstance* instance
);
static void os_wasm_library_instance_free(
    OsWasmLibraryInstance* instance
);
static OsStatus os_wasm_library_evict_until(
    uint64_t required_bytes,
    OsWasmLibraryInstance* excluded_instance
);
static OsStatus os_wasm_library_instance_load(
    OsTaskHandle task,
    OsWasmLibraryHandle library,
    OsWasmLibraryInstance** out_instance
);
static void os_wasm_library_task_cleanup(OsTaskHandle task);
static void os_wasm_library_import_free_all(OsTaskHandle task);
static M3Result os_link_wasm_library_imports(OsTaskHandle task);
static m3ApiRawFunction(os_wasm_import_library_call);
static void os_wasm_library_pending_call_free(OsTaskHandle task);
static OsTaskRunResult os_run_wasm_library_call(
    OsTaskHandle task,
    uint64_t* out_remaining_fuel
);
...
static m3ApiRawFunction(os_wasm_import_os_timer_create);
static m3ApiRawFunction(os_wasm_import_os_timer_delete);
static m3ApiRawFunction(os_wasm_import_os_timer_start);
static m3ApiRawFunction(os_wasm_import_os_timer_stop);
static m3ApiRawFunction(os_wasm_import_os_timer_reset);
static m3ApiRawFunction(os_wasm_import_os_timer_change_period);
static m3ApiRawFunction(os_wasm_import_os_timer_is_active);
...
OsStatus os_init(void)
{
    ...
    g_os.next_wasm_library_use_sequence = 1U;
    g_os.wasm_library_cache_limit_bytes = OS_WASM_LIBRARY_CACHE_UNLIMITED;
    g_os.initialized = 1U;
    os_clear_last_error();
...
void os_shutdown(void)
{
    ...
    if (g_os.clock_wakeup_armed && g_os.clock_port.cancel_wakeup != NULL)
    {
        g_os.clock_port.cancel_wakeup(g_os.clock_port.context);
    }
...
)
{
    ...
    if (out_library == NULL || module_name == NULL || module_name[0] == '\0' ||
        source == NULL || source->acquire == NULL || source->release == NULL ||
        (policy != OS_WASM_LIBRARY_EVICTABLE &&
         policy != OS_WASM_LIBRARY_PINNED))
    {
        return OS_STATUS_INVALID_ARGUMENT;
    }
    ...
    *out_library = NULL;
    ...
    if (strcmp(module_name, "*") == 0 ||
        strcmp(module_name, "env") == 0 ||
        strcmp(module_name, "wasi_unstable") == 0 ||
        strcmp(module_name, "wasi_snapshot_preview1") == 0 ||
        os_wasm_library_find(module_name) != NULL ||
        os_wasm_library_has_host_import_module(module_name))
    {
        return OS_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (library->source.stack_size == 0U)
    {
        library->source.stack_size = OS_WASM_LIBRARY_DEFAULT_STACK_SIZE;
    }

    status = os_wasm_library_validate_source(library);
    if (status != OS_STATUS_OK)
    {
        os_wasm_library_free(library);
        return status;
    }

    os_wasm_library_list_insert(library);
    *out_library = library;
    return OS_STATUS_OK;
}
...
{
    OsWasmLibraryInstance* instance = NULL;
    ...
    if (!os_wasm_library_is_in_list(library))
    {
        return OS_STATUS_WASM_LIBRARY_NOT_FOUND;
    }

    if (library->binding_count != 0U)
    {
        return OS_STATUS_BUSY;
    }

    for (instance = g_os.wasm_library_instance_list;
         instance != NULL;
    ...
    os_wasm_library_list_remove(library);
    os_wasm_library_free(library);
    return OS_STATUS_OK;
}
...
{
    return os_wasm_library_is_in_list(library) ? library->id : 0U;
}

const char* os_wasm_library_get_name(OsWasmLibraryHandle library)
{
    return os_wasm_library_is_in_list(library)
        ? library->module_name
        : NULL;
}
...
{
    uint64_t previous_limit = g_os.wasm_library_cache_limit_bytes;
    OsStatus status = OS_STATUS_OK;
    ...
    g_os.wasm_library_cache_limit_bytes = limit_bytes;
    status = os_wasm_library_evict_until(0U, NULL);
    if (status != OS_STATUS_OK)
    {
        g_os.wasm_library_cache_limit_bytes = previous_limit;
        return status;
    }
...
{
    return g_os.wasm_library_cache_limit_bytes;
}

uint64_t os_wasm_library_cache_get_resident_size(void)
{
    return g_os.wasm_library_resident_size_bytes;
}
...
{
    if (task == NULL || library == NULL ||
        !os_task_is_in_list(task) ||
        !os_wasm_library_is_in_list(library))
    {
        return 0U;
    }

    return os_wasm_library_instance_find(task, library) != NULL ? 1U : 0U;
}
...
)
{
    ...
    instance = os_wasm_library_instance_find(task, library);
    if (instance == NULL)
    {
        return OS_STATUS_WASM_LIBRARY_NOT_FOUND;
    }

    if (instance->active || library->policy == OS_WASM_LIBRARY_PINNED)
    {
        return OS_STATUS_BUSY;
    }

    os_wasm_library_instance_free(instance);
    return OS_STATUS_OK;
}
...
)
{
    ...
    if (out_timer == NULL || period_ms == 0U || period_ms > (uint32_t)INT32_MAX ||
        target_task == NULL ||
        notify_action < OS_NOTIFY_NO_ACTION ||
        notify_action > OS_NOTIFY_SET_VALUE_WITHOUT_OVERWRITE)
    {
        return OS_STATUS_INVALID_ARGUMENT;
    }
    ...
    *out_timer = NULL;
    ...
    if (!os_task_is_in_list(target_task))
    {
        return OS_STATUS_TASK_NOT_FOUND;
    }

    if (os_task_is_dead(target_task))
    {
        return OS_STATUS_TASK_DEAD;
    }

    timer = os_timer_allocate();
    if (timer == NULL)
    {
        return OS_STATUS_OUT_OF_MEMORY;
    }

    timer->id = g_os.next_timer_id++;
    timer->period_ms = period_ms;
    timer->target_task = target_task;
    timer->notify_value = notify_value;
    timer->notify_action = notify_action;
    timer->auto_reload = auto_reload ? 1U : 0U;
    os_timer_list_insert(timer);
    ++g_os.timer_count;
    *out_timer = timer;
    return OS_STATUS_OK;
}
...
void os_timer_delete(OsTimerHandle timer)
{
    ...
    os_timer_list_remove(timer);
    os_timer_free(timer);
    if (g_os.timer_count > 0U)
    {
        --g_os.timer_count;
    }
    os_clock_rearm();
...
{
    return os_timer_is_in_list(timer) ? timer->id : 0U;
}
...
OsStatus os_timer_start(OsTimerHandle timer)
{
    ...
    timer->expiry_tick_ms = g_os.tick_ms + timer->period_ms;
    timer->active = 1U;
    os_clock_rearm();
    return OS_STATUS_OK;
}
...
OsStatus os_timer_change_period(OsTimerHandle timer, uint32_t period_ms)
{
    ...
    timer->period_ms = period_ms;
    timer->expiry_tick_ms = g_os.tick_ms + period_ms;
    timer->active = 1U;
    os_clock_rearm();
    return OS_STATUS_OK;
}
...
{
    return os_timer_is_in_list(timer) ? timer->active : 0U;
}

uint32_t os_timer_get_period_ms(OsTimerHandle timer)
{
    return os_timer_is_in_list(timer) ? timer->period_ms : 0U;
}
...
OsStatus os_clock_port_set(const OsClockPort* port)
{
    ...
    if (port == NULL || port->now_ms == NULL ||
        ((port->arm_wakeup == NULL) != (port->cancel_wakeup == NULL)))
    {
        return OS_STATUS_INVALID_ARGUMENT;
    }
...
void os_clock_port_clear(void)
{
    ...
    memset(&g_os.clock_port, 0, sizeof(g_os.clock_port));
    g_os.clock_wakeup_armed = 0U;
    g_os.clock_armed_tick_ms = 0U;
    g_os.time_update_initialized = 0U;
}
...
OsStatus os_clock_poll(void)
{
    ...
    now_ms = g_os.clock_port.now_ms(g_os.clock_port.context);
    return os_update_time_ms(now_ms);
}

OsStatus os_clock_wakeup(void)
{
    if (!os_clock_is_tickless())
    {
        return OS_STATUS_UNSUPPORTED;
    }
...
{
    return g_os.clock_port.now_ms != NULL &&
           g_os.clock_port.arm_wakeup != NULL &&
           g_os.clock_port.cancel_wakeup != NULL;
}
...
uint32_t os_get_next_wakeup_ms(void)
{
    OsTaskHandle task = NULL;
    OsTimerHandle timer = NULL;
    uint32_t next_wakeup_ms = OS_WAIT_FOREVER;

    for (task = g_os.task_list; task != NULL; task = task->next)
    {
        uint32_t remaining_ms = 0U;

        if (task->state != OS_TASK_WAITING || !task->wait_has_timeout)
        {
            continue;
        }

        remaining_ms = os_time_reached(g_os.tick_ms, task->wake_tick_ms)
            ? 0U
            : task->wake_tick_ms - g_os.tick_ms;
        if (remaining_ms < next_wakeup_ms)
        {
            next_wakeup_ms = remaining_ms;
        }
    }
    ...
    for (timer = g_os.timer_list; timer != NULL; timer = timer->next)
    {
        ...
        remaining_ms = os_time_reached(g_os.tick_ms, timer->expiry_tick_ms)
            ? 0U
            : timer->expiry_tick_ms - g_os.tick_ms;
        if (remaining_ms < next_wakeup_ms)
        {
            next_wakeup_ms = remaining_ms;
        }
...
OsStatus os_schedule(void)
{
    ...
    switch (run_result)
    {
        ...
        default:
            ...
            schedule_status = g_os.last_error.status != OS_STATUS_OK
                ? g_os.last_error.status
...
static OsTimerHandle os_timer_allocate(void)
{
    return (OsTimerHandle)calloc(1U, sizeof(struct OsTimer));
}
...
static void os_timer_list_insert(OsTimerHandle timer)
{
    ...
    if (g_os.timer_list == NULL)
    {
        g_os.timer_list = timer;
        return;
    }

    for (cursor = g_os.timer_list; cursor->next != NULL; cursor = cursor->next)
    {
    }

    cursor->next = timer;
    timer->previous = cursor;
}
...
static void os_timer_list_remove(OsTimerHandle timer)
{
    ...
    else if (g_os.timer_list == timer)
    {
        g_os.timer_list = timer->next;
    }
...
static int os_timer_is_in_list(OsTimerHandle timer)
{
    ...
    for (cursor = g_os.timer_list; cursor != NULL; cursor = cursor->next)
    {
        if (cursor == timer)
        {
            return 1;
...
static void os_timer_free_all(void)
{
    OsTimerHandle timer = g_os.timer_list;

    while (timer != NULL)
    {
        OsTimerHandle next = timer->next;
        os_timer_free(timer);
        timer = next;
    }

    g_os.timer_list = NULL;
    g_os.timer_count = 0U;
}
...
static uint8_t os_task_snapshot_is_busy(OsTaskHandle task)
{
    ...
    if (task->pending_wasm_library_call != NULL)
    {
        return 1U;
    }
...
static OsValueType os_wasm_library_value_type(M3ValueType type)
{
    ...
    {
        case c_m3Type_i32:
            return OS_VALUE_TYPE_I32;
        case c_m3Type_i64:
            return OS_VALUE_TYPE_I64;
        case c_m3Type_f32:
            return OS_VALUE_TYPE_F32;
        case c_m3Type_f64:
            return OS_VALUE_TYPE_F64;
        default:
            return (OsValueType)0;
    }
}
...
)
{
    ...
    if (function == NULL || library_export == NULL ||
        m3_GetArgCount(function) != library_export->argument_count ||
        m3_GetRetCount(function) != library_export->return_count)
    {
        return 0U;
    }

    for (type_index = 0U;
         type_index < library_export->return_count;
         ++type_index)
    {
        if (os_wasm_library_value_type(
                m3_GetRetType(function, type_index)
            ) != library_export->return_types[type_index])
        {
            return 0U;
        }
    }
    ...
    for (type_index = 0U;
         type_index < library_export->argument_count;
         ++type_index)
    {
        if (os_wasm_library_value_type(
                m3_GetArgType(function, type_index)
            ) != library_export->argument_types[type_index])
        {
            return 0U;
        }
    }
...
static int os_wasm_library_has_host_import_module(const char* module_name)
{
    ...
    for (node = g_os.host_import_list; node != NULL; node = node->next)
    {
        if (strcmp(node->module_name, "*") == 0 ||
            strcmp(node->module_name, module_name) == 0)
        {
            return 1;
        }
    }
...
static void os_wasm_library_export_free_all(OsWasmLibraryExport* library_export)
{
    while (library_export != NULL)
    {
        OsWasmLibraryExport* next = library_export->next;

        free(library_export->name);
        free(library_export->return_types);
        free(library_export->argument_types);
        free(library_export);
        library_export = next;
    }
}
...
{
    OsWasmLibraryExport* library_export = NULL;
    ...
    for (library_export = library->exports;
         library_export != NULL;
         library_export = library_export->next)
    {
        if (strcmp(library_export->name, export_name) == 0)
        {
...
)
{
    ...
    uint32_t wasm_size = 0U;
    IM3Environment environment = NULL;
    IM3Module module = NULL;
    OsWasmLibraryExport* export_list = NULL;
    OsWasmLibraryExport* export_tail = NULL;
    OsStatus status = OS_STATUS_OK;
    M3Result result = m3Err_none;
    uint8_t source_acquired = 0U;
    ...
    if (environment == NULL)
    {
        status = OS_STATUS_OUT_OF_MEMORY;
        goto cleanup;
    }

    result = m3_ParseModule(environment, &module, wasm_bytes, wasm_size);
    ...
    if (module->numFuncImports != 0U || module->memoryImported)
    {
        result = "WASM library imports are unsupported";
        status = OS_STATUS_UNSUPPORTED;
        goto cleanup;
    }

    for (global_index = 0U;
         global_index < module->numGlobals;
         ++global_index)
    {
        if (module->globals[global_index].imported)
        {
            result = "WASM library global imports are unsupported";
            status = OS_STATUS_UNSUPPORTED;
            goto cleanup;
        }
    }
    ...
    for (function_index = 0U;
         function_index < module->numFunctions;
         ++function_index)
    {
        IM3Function function = &module->functions[function_index];
        OsWasmLibraryExport* library_export = NULL;
        uint32_t type_index = 0U;

        if (function->export_name == NULL ||
            function->export_name[0] == '\0')
        {
            continue;
        }

        if (os_wasm_library_export_find(library, function->export_name) != NULL)
        {
            result = "duplicate WASM library export";
            status = OS_STATUS_WASM_ERROR;
            goto cleanup;
        }

        for (library_export = export_list;
             library_export != NULL;
             library_export = library_export->next)
        {
            if (strcmp(library_export->name, function->export_name) == 0)
            {
                result = "duplicate WASM library export";
                status = OS_STATUS_WASM_ERROR;
                goto cleanup;
        ...
        library_export = (OsWasmLibraryExport*)calloc(
            1U,
            sizeof(OsWasmLibraryExport)
        );
        if (library_export == NULL)
        {
            status = OS_STATUS_OUT_OF_MEMORY;
            goto cleanup;
        }

        library_export->name = os_duplicate_string(function->export_name);
        library_export->return_count = m3_GetRetCount(function);
        library_export->argument_count = m3_GetArgCount(function);

        if (library_export->return_count > 0U)
        {
            library_export->return_types = (OsValueType*)calloc(
                library_export->return_count,
                sizeof(OsValueType)
            );
        }
        if (library_export->argument_count > 0U)
        {
            library_export->argument_types = (OsValueType*)calloc(
                library_export->argument_count,
                sizeof(OsValueType)
            );
        }

        if (library_export->name == NULL ||
            (library_export->return_count > 0U &&
             library_export->return_types == NULL) ||
            (library_export->argument_count > 0U &&
             library_export->argument_types == NULL))
        {
            os_wasm_library_export_free_all(library_export);
            status = OS_STATUS_OUT_OF_MEMORY;
            goto cleanup;
        }
        ...
        {
            library_export->return_types[type_index] =
                os_wasm_library_value_type(
                    m3_GetRetType(function, type_index)
                );
            if (library_export->return_types[type_index] == 0)
            {
                os_wasm_library_export_free_all(library_export);
                result = "unsupported WASM library result type";
                status = OS_STATUS_UNSUPPORTED;
                goto cleanup;
            }
        }
             ...
             ++type_index)
        {
            ...
            if (library_export->argument_types[type_index] == 0)
            {
                os_wasm_library_export_free_all(library_export);
                result = "unsupported WASM library argument type";
                status = OS_STATUS_UNSUPPORTED;
                goto cleanup;
            }
        }
        ...
        if (export_tail != NULL)
        {
            export_tail->next = library_export;
        }
        else
        {
            export_list = library_export;
        }
        export_tail = library_export;
    }

    if (export_list == NULL)
    {
        result = "WASM library has no exported functions";
        status = OS_STATUS_WASM_EXPORT_NOT_FOUND;
        goto cleanup;
    }

    library->exports = export_list;
    export_list = NULL;

cleanup:
    os_wasm_library_export_free_all(export_list);
...
static void os_wasm_library_free(OsWasmLibraryHandle library)
{
    ...
    os_wasm_library_export_free_all(library->exports);
    library->exports = NULL;
    free(library->module_name);
    library->module_name = NULL;
    free(library);
}

static void os_wasm_library_free_all(void)
{
    while (g_os.wasm_library_instance_list != NULL)
    {
        os_wasm_library_instance_free(g_os.wasm_library_instance_list);
    }

    while (g_os.wasm_library_list != NULL)
    {
...
static int os_wasm_library_is_in_list(OsWasmLibraryHandle library)
{
    OsWasmLibraryHandle cursor = NULL;
    ...
    for (cursor = g_os.wasm_library_list;
         cursor != NULL;
         cursor = cursor->next)
    {
        if (cursor == library)
...
static void os_wasm_library_list_insert(OsWasmLibraryHandle library)
{
    ...
    library->previous = NULL;
    library->next = NULL;
    if (g_os.wasm_library_list == NULL)
    {
...
static void os_wasm_library_list_remove(OsWasmLibraryHandle library)
{
    ...
    {
        library->previous->next = library->next;
    }
    else if (g_os.wasm_library_list == library)
    {
        g_os.wasm_library_list = library->next;
    }

    if (library->next != NULL)
    {
        library->next->previous = library->previous;
    }
...
)
{
    ...
    for (instance = g_os.wasm_library_instance_list;
         ...
         instance = instance->next)
    {
        if (instance->task == task && instance->library == library)
        {
            return instance;
        }
    }
...
{
    OsWasmLibraryInstance* cursor = NULL;

    if (instance == NULL || instance->listed)
    {
        return;
    }
    ...
    instance->next = NULL;
    if (g_os.wasm_library_instance_list == NULL)
    {
        g_os.wasm_library_instance_list = instance;
    }
    else
    {
        for (cursor = g_os.wasm_library_instance_list;
             cursor->next != NULL;
             cursor = cursor->next)
        {
        }
        cursor->next = instance;
        instance->previous = cursor;
    }

    instance->listed = 1U;
    if (UINT64_MAX - g_os.wasm_library_resident_size_bytes <
        instance->resident_size_bytes)
    {
        g_os.wasm_library_resident_size_bytes = UINT64_MAX;
    }
    else
    {
        g_os.wasm_library_resident_size_bytes +=
            instance->resident_size_bytes;
    }
}
...
)
{
    ...
    else if (g_os.wasm_library_instance_list == instance)
    {
        g_os.wasm_library_instance_list = instance->next;
    }
    ...
    if (g_os.wasm_library_resident_size_bytes >=
        instance->resident_size_bytes)
    {
        g_os.wasm_library_resident_size_bytes -=
            instance->resident_size_bytes;
    ...
    {
        g_os.wasm_library_resident_size_bytes = 0U;
    }
...
)
{
    ...
    if (instance->library->source.resident_size_bytes != 0U)
    {
        return instance->library->source.resident_size_bytes;
    }
    ...
    {
        size += (uint64_t)sizeof(*instance->wasm_environment);
    }
    ...
    {
        size += (uint64_t)sizeof(*instance->wasm_module);
        size += (uint64_t)instance->wasm_module->numFunctions *
            (uint64_t)sizeof(M3Function);
        size += (uint64_t)instance->wasm_module->numGlobals *
            (uint64_t)sizeof(M3Global);
        size += (uint64_t)instance->wasm_module->numDataSegments *
            (uint64_t)sizeof(M3DataSegment);
    }
...
)
{
    ...
    new_size = os_wasm_library_runtime_size(instance);
    if (new_size == instance->resident_size_bytes)
    {
        return;
    }
    ...
    if (instance->listed)
    {
        ...
        if (UINT64_MAX - g_os.wasm_library_resident_size_bytes < new_size)
        {
            g_os.wasm_library_resident_size_bytes = UINT64_MAX;
        }
        else
        {
            g_os.wasm_library_resident_size_bytes += new_size;
        }
    }

    instance->resident_size_bytes = new_size;
}
...
)
{
    ...
    instance->last_used_sequence = g_os.next_wasm_library_use_sequence++;
    if (g_os.next_wasm_library_use_sequence == 0U)
    {
        g_os.next_wasm_library_use_sequence = 1U;
    }
}
...
)
{
    ...
    os_wasm_library_instance_remove(instance);

    if (instance->wasm_runtime != NULL)
    {
        m3_FreeRuntime(instance->wasm_runtime);
        instance->wasm_runtime = NULL;
    }

    if (instance->wasm_module != NULL &&
        !instance->wasm_module_loaded)
    {
        m3_FreeModule(instance->wasm_module);
    }
    instance->wasm_module = NULL;
    instance->wasm_module_loaded = 0U;

    if (instance->wasm_environment != NULL)
    {
        m3_FreeEnvironment(instance->wasm_environment);
        instance->wasm_environment = NULL;
    }

    if (instance->source_acquired &&
        instance->library != NULL)
    {
        instance->library->source.release(
            instance->library->source.context,
            instance->wasm_bytes,
            instance->wasm_size
        );
    }
    instance->source_acquired = 0U;
    instance->wasm_bytes = NULL;
    instance->wasm_size = 0U;
    free(instance);
}
...
)
{
    ...
    uint64_t required_reclaim_bytes = 0U;
    uint64_t target_size = 0U;

    if (g_os.wasm_library_cache_limit_bytes ==
        OS_WASM_LIBRARY_CACHE_UNLIMITED)
    {
        return OS_STATUS_OK;
    }

    if (required_bytes > g_os.wasm_library_cache_limit_bytes)
    {
        return OS_STATUS_WASM_CACHE_FULL;
    }

    target_size = g_os.wasm_library_cache_limit_bytes - required_bytes;
    if (g_os.wasm_library_resident_size_bytes <= target_size)
    {
        return OS_STATUS_OK;
    }

    required_reclaim_bytes =
        g_os.wasm_library_resident_size_bytes - target_size;
    for (instance = g_os.wasm_library_instance_list;
         instance != NULL;
         instance = instance->next)
    {
        if (instance == excluded_instance ||
            instance->active ||
            instance->library == NULL ||
            instance->library->policy != OS_WASM_LIBRARY_EVICTABLE)
        {
            continue;
        }

        if (UINT64_MAX - reclaimable_bytes <
            instance->resident_size_bytes)
        {
            reclaimable_bytes = UINT64_MAX;
            break;
        }
        reclaimable_bytes += instance->resident_size_bytes;
    }

    if (reclaimable_bytes < required_reclaim_bytes)
    {
        return OS_STATUS_WASM_CACHE_FULL;
    }

    while (g_os.wasm_library_resident_size_bytes > target_size)
    {
        OsWasmLibraryInstance* candidate = NULL;
        ...
        os_wasm_library_instance_free(candidate);
    }
...
)
{
    ...
    OsStatus status = OS_STATUS_OK;
    M3Result result = m3Err_none;
    uint64_t size_hint = 0U;
    uint32_t global_index = 0U;

    if (task == NULL || library == NULL || out_instance == NULL)
    {
        return OS_STATUS_INVALID_ARGUMENT;
    }
    ...
    *out_instance = NULL;
    ...
    if (instance != NULL)
    {
        os_wasm_library_instance_touch(instance);
        *out_instance = instance;
        return OS_STATUS_OK;
    }

    size_hint = library->source.resident_size_bytes;
    status = os_wasm_library_evict_until(size_hint, NULL);
    if (status != OS_STATUS_OK)
    {
        return status;
    }

    instance = (OsWasmLibraryInstance*)calloc(
        1U,
        sizeof(OsWasmLibraryInstance)
    );
    if (instance == NULL)
    {
        return OS_STATUS_OUT_OF_MEMORY;
    ...
    instance->source_acquired = 1U;

    if (instance->wasm_bytes == NULL || instance->wasm_size == 0U)
    {
        os_wasm_library_instance_free(instance);
        return OS_STATUS_INVALID_ARGUMENT;
    }

    instance->wasm_environment = m3_NewEnvironment();
    if (instance->wasm_environment == NULL)
    {
        os_wasm_library_instance_free(instance);
        return OS_STATUS_OUT_OF_MEMORY;
    ...
    instance->wasm_runtime = m3_NewRuntime(
        instance->wasm_environment,
        library->source.stack_size,
        task
    );
    if (instance->wasm_runtime == NULL)
    {
    ...
    {
        status = os_wasm_result_to_status(result);
        os_set_last_error(task, "parse_wasm_library", result, status);
        os_wasm_library_instance_free(instance);
        return status;
    }

    if (instance->wasm_module->numFuncImports != 0U ||
        instance->wasm_module->memoryImported)
    {
        os_set_last_error(
            task,
            "validate_wasm_library",
            "WASM library imports are unsupported",
    ...
    for (global_index = 0U;
         global_index < instance->wasm_module->numGlobals;
         ++global_index)
    {
        if (instance->wasm_module->globals[global_index].imported)
        {
            os_set_last_error(
                task,
                "validate_wasm_library",
                "WASM library global imports are unsupported",
                OS_STATUS_UNSUPPORTED
            );
            os_wasm_library_instance_free(instance);
            return OS_STATUS_UNSUPPORTED;
    ...
    m3_SetModuleName(instance->wasm_module, library->module_name);
    result = m3_LoadModule(
        instance->wasm_runtime,
        instance->wasm_module
    );
    if (result != m3Err_none)
    {
        status = os_wasm_result_to_status(result);
        os_set_last_error(task, "load_wasm_library", result, status);
        os_wasm_library_instance_free(instance);
        return status;
    }
    instance->wasm_module_loaded = 1U;

    instance->resident_size_bytes =
        os_wasm_library_runtime_size(instance);
    status = os_wasm_library_evict_until(
        instance->resident_size_bytes,
        NULL
    );
    if (status != OS_STATUS_OK)
    {
        os_set_last_error(
            task,
            "cache_wasm_library",
            "WASM library cache limit exceeded",
            status
        );
        os_wasm_library_instance_free(instance);
        return status;
    ...
    os_wasm_library_instance_touch(instance);
    os_wasm_library_instance_insert(instance);
    *out_instance = instance;
    return OS_STATUS_OK;
}
...
static void os_wasm_library_pending_call_free(OsTaskHandle task)
{
    OsPendingWasmLibraryCall* pending_call = NULL;

    if (task == NULL || task->pending_wasm_library_call == NULL)
    {
        return;
    }

    pending_call = task->pending_wasm_library_call;
    if (pending_call->instance != NULL)
    {
        pending_call->instance->active = 0U;
    }

    free(pending_call->values);
    free(pending_call->argument_pointers);
    free(pending_call->return_pointers);
    free(pending_call);
    task->pending_wasm_library_call = NULL;
}

static void os_wasm_library_import_free_all(OsTaskHandle task)
{
    OsWasmLibraryImportBinding* binding = NULL;
    ...
    binding = task->wasm_library_imports;
    while (binding != NULL)
    {
        OsWasmLibraryImportBinding* next = binding->next;

        if (binding->library != NULL &&
            binding->library->binding_count > 0U)
        {
            --binding->library->binding_count;
        }
        free(binding);
        binding = next;
    }

    task->wasm_library_imports = NULL;
}
...
static void os_wasm_library_task_cleanup(OsTaskHandle task)
{
    ...
    os_wasm_library_pending_call_free(task);

    instance = g_os.wasm_library_instance_list;
    while (instance != NULL)
    {
        OsWasmLibraryInstance* next = instance->next;
    ...
    os_wasm_library_import_free_all(task);
}
...
static M3Result os_link_wasm_library_imports(OsTaskHandle task)
{
    ...
    for (function_index = 0U;
         function_index < task->wasm_module->numFuncImports;
         ++function_index)
    {
        IM3Function import_function =
            &task->wasm_module->functions[function_index];
        OsWasmLibraryHandle library = NULL;
        OsWasmLibraryExport* library_export = NULL;
        OsWasmLibraryImportBinding* binding = NULL;
        M3Result result = m3Err_none;

        if (import_function->import.moduleUtf8 == NULL ||
            import_function->import.fieldUtf8 == NULL)
        {
            continue;
        }

        library = os_wasm_library_find(
            import_function->import.moduleUtf8
        );
        if (library == NULL)
        {
            continue;
        ...
        library_export = os_wasm_library_export_find(
            library,
            import_function->import.fieldUtf8
        );
        if (library_export == NULL)
        {
            return OS_WASM_LIBRARY_EXPORT_NOT_FOUND_ERROR;
        }

        if (!os_wasm_library_function_matches_export(
                import_function,
                library_export
            ))
        {
            return OS_WASM_LIBRARY_SIGNATURE_MISMATCH_ERROR;
        }

        for (binding = task->wasm_library_imports;
             binding != NULL;
             binding = binding->next)
        {
            if (binding->library == library &&
                binding->library_export == library_export)
            {
                break;
            }
        }
        ...
        binding = (OsWasmLibraryImportBinding*)calloc(
            1U,
            sizeof(OsWasmLibraryImportBinding)
        );
        if (binding == NULL)
        {
        ...
        binding->library_export = library_export;
        binding->next = task->wasm_library_imports;
        task->wasm_library_imports = binding;
        ++library->binding_count;
...
static m3ApiRawFunction(os_wasm_import_library_call)
{
    ...
    uintptr_t stack_start_address = 0U;
    uintptr_t stack_end_address = 0U;
    uintptr_t call_start_address = (uintptr_t)_sp;
    uint64_t slot_count = 0U;
    uint64_t call_size = 0U;
    uint32_t value_index = 0U;
    (void)_mem;

    if (runtime == NULL || _ctx == NULL ||
        _sp == NULL || _ctx->userdata == NULL)
    {
        return m3Err_trapAbort;
    }

    binding = (OsWasmLibraryImportBinding*)_ctx->userdata;
    task = binding->task;
    if (task == NULL || task != g_os.current_task ||
        task->wasm_runtime != runtime ||
        task->pending_wasm_library_call != NULL ||
        binding->library == NULL ||
        binding->library_export == NULL)
    {
        return m3Err_trapAbort;
    }

    slot_count =
        (uint64_t)binding->library_export->return_count +
        (uint64_t)binding->library_export->argument_count;
    if (slot_count > (uint64_t)SIZE_MAX / sizeof(uint64_t))
    {
        return m3Err_mallocFailed;
    }
    call_size = slot_count * sizeof(uint64_t);
    ...
    stack_start_address = (uintptr_t)stack_start;
    if ((uintptr_t)runtime->stackSize >
        UINTPTR_MAX - stack_start_address)
    {
        return m3Err_trapOutOfBoundsMemoryAccess;
    }
    stack_end_address =
        stack_start_address + (uintptr_t)runtime->stackSize;

    if (call_start_address < stack_start_address ||
        call_start_address > stack_end_address ||
        call_size >
            (uint64_t)(stack_end_address - call_start_address))
    {
        return m3Err_trapOutOfBoundsMemoryAccess;
    }

    pending_call = (OsPendingWasmLibraryCall*)calloc(
        1U,
        sizeof(OsPendingWasmLibraryCall)
    );
    ...
    pending_call->binding = binding;
    pending_call->return_offset =
        (uint32_t)(call_start_address - stack_start_address);
    pending_call->values = (uint64_t*)calloc(
        slot_count > 0U ? (size_t)slot_count : 1U,
        sizeof(uint64_t)
    );
    if (binding->library_export->argument_count > 0U)
    {
        pending_call->argument_pointers = (const void**)calloc(
            binding->library_export->argument_count,
            sizeof(const void*)
        );
    }
    if (binding->library_export->return_count > 0U)
    {
        pending_call->return_pointers = (const void**)calloc(
            binding->library_export->return_count,
            sizeof(const void*)
        );
    }

    if (pending_call->values == NULL ||
        (binding->library_export->argument_count > 0U &&
         pending_call->argument_pointers == NULL) ||
        (binding->library_export->return_count > 0U &&
         pending_call->return_pointers == NULL))
    {
        free(pending_call->values);
        free(pending_call->argument_pointers);
        free(pending_call->return_pointers);
    ...
    for (value_index = 0U;
         value_index < binding->library_export->return_count;
         ++value_index)
    {
        pending_call->return_pointers[value_index] =
            &pending_call->values[value_index];
    }

    for (value_index = 0U;
         value_index < binding->library_export->argument_count;
         ++value_index)
    {
        uint32_t slot_index =
            binding->library_export->return_count + value_index;

        pending_call->values[slot_index] = _sp[slot_index];
        pending_call->argument_pointers[value_index] =
            &pending_call->values[slot_index];
    }

    task->pending_wasm_library_call = pending_call;
    m3_SetFuel(runtime, 0U);
    return m3Err_none;
}
...
)
{
    ...
    OsStatus status = OS_STATUS_OK;
    M3Result result = m3Err_none;
    uint64_t remaining_fuel = 0U;
    uint64_t result_size = 0U;
    uint8_t ran_start_stage = 0U;

    if (task == NULL || out_remaining_fuel == NULL ||
        task->pending_wasm_library_call == NULL)
    {
        return OS_TASK_RUN_ERROR;
    }

    *out_remaining_fuel = 0U;
    pending_call = task->pending_wasm_library_call;
    if (pending_call->binding == NULL ||
        pending_call->binding->library == NULL ||
        pending_call->binding->library_export == NULL)
    {
        os_set_last_error(
            task,
            "call_wasm_library",
            "invalid pending WASM library call",
            OS_STATUS_WASM_ERROR
        );
        return OS_TASK_RUN_ERROR;
    }
    library_export = pending_call->binding->library_export;

    if (pending_call->instance == NULL)
    {
        status = os_wasm_library_instance_load(
            task,
            pending_call->binding->library,
            &pending_call->instance
        );
        if (status != OS_STATUS_OK)
        {
            if (g_os.last_error.status == OS_STATUS_OK)
            {
                os_set_last_error(
                    task,
                    "load_wasm_library",
                    "WASM library load failed",
                    status
                );
            }
            return OS_TASK_RUN_ERROR;
    ...
    instance = pending_call->instance;
    instance->active = 1U;
    os_wasm_library_instance_touch(instance);

    if (pending_call->function == NULL)
    {
        ...
        if (m3_GetFunctionModule(pending_call->function) !=
                instance->wasm_module ||
            pending_call->function->export_name == NULL ||
            strcmp(
                pending_call->function->export_name,
                library_export->name
            ) != 0)
        {
            os_set_last_error(
        ...
        os_wasm_library_instance_refresh_size(instance);
        status = os_wasm_library_evict_until(0U, instance);
        if (status != OS_STATUS_OK)
        {
            os_set_last_error(
                task,
    ...
    m3_SetFuel(instance->wasm_runtime, OS_TASK_SLICE_FUEL);
    if (g_os.preempt_requested)
    {
        m3_SetFuel(instance->wasm_runtime, 0U);
    }

    if (pending_call->start_suspended)
    {
        ran_start_stage = 1U;
        result = m3_Resume(instance->wasm_runtime);
    }
    else if (instance->wasm_module->startFunction >= 0)
    {
        ran_start_stage = 1U;
        result = m3_RunStart(instance->wasm_module);
    }

    if (ran_start_stage)
    {
        remaining_fuel = m3_GetFuel(instance->wasm_runtime);
        os_wasm_library_instance_refresh_size(instance);
        status = os_wasm_library_evict_until(0U, instance);
        if (status != OS_STATUS_OK)
        {
        ...
        {
            pending_call->start_suspended = 1U;
            os_clear_last_error();
        ...
        /*
         * m3_RunStart restores startFunction when fuel is exhausted. A
         * successful m3_Resume finishes that same invocation, so mark it
         * consumed here instead of allowing m3_Call to run it a second time.
         */
        instance->wasm_module->startFunction = -1;
        pending_call->start_suspended = 0U;
        ...
        m3_SetFuel(instance->wasm_runtime, remaining_fuel);
    }

    if (!pending_call->started)
    {
        pending_call->started = 1U;
        result = m3_Call(
            pending_call->function,
            library_export->argument_count,
            pending_call->argument_pointers
        );
    }
    else
    {
    ...
    result = m3_GetResults(
        pending_call->function,
        library_export->return_count,
        pending_call->return_pointers
    );
    if (result != m3Err_none)
    {
        os_set_last_error(
            task,
            "get_wasm_library_results",
            result,
            OS_STATUS_WASM_ERROR
        );
        return OS_TASK_RUN_ERROR;
    ...
    result_size =
        (uint64_t)library_export->return_count * sizeof(uint64_t);
    if (task->wasm_stack_base == NULL ||
        pending_call->return_offset > task->stack_size ||
        result_size >
            (uint64_t)(task->stack_size - pending_call->return_offset))
    {
        os_set_last_error(
            task,
            "store_wasm_library_results",
    ...
    {
        memcpy(
            task->wasm_stack_base + pending_call->return_offset,
            pending_call->values,
            (size_t)result_size
        );
    }
    ...
    os_clear_last_error();
    *out_remaining_fuel = remaining_fuel;
    return OS_TASK_RUN_LIBRARY_COMPLETE;
}
...
static void os_task_cleanup_wasm(OsTaskHandle task)
{
    ...
    os_wasm_library_task_cleanup(task);
...
static M3Result os_link_task_host_imports(OsTaskHandle task)
{
    ...
    } imports[] =
    {
        ...
        { "os_timer_create", "i(iiiii)", os_wasm_import_os_timer_create },
        { "os_timer_delete", "i(i)", os_wasm_import_os_timer_delete },
        { "os_timer_start", "i(i)", os_wasm_import_os_timer_start },
        { "os_timer_stop", "i(i)", os_wasm_import_os_timer_stop },
        { "os_timer_reset", "i(i)", os_wasm_import_os_timer_reset },
        { "os_timer_change_period", "i(ii)", os_wasm_import_os_timer_change_period },
        { "os_timer_is_active", "i(i)", os_wasm_import_os_timer_is_active }
    };
    ...
    result = os_link_wasm_library_imports(task);
    if (result != m3Err_none)
    {
        return result;
    }
...
static m3ApiRawFunction(os_wasm_import_os_timer_create)
{
    m3ApiReturnType(uint32_t);
    m3ApiGetArg(uint32_t, period_ms);
    m3ApiGetArg(uint32_t, auto_reload);
    m3ApiGetArg(uint32_t, target_task_id);
    m3ApiGetArg(uint32_t, notify_value);
    m3ApiGetArg(uint32_t, notify_action);
    OsTaskHandle target_task = target_task_id == 0U
        ? os_task_get_current()
        : os_task_find_by_id(target_task_id);
    OsTimerHandle timer = NULL;
    OsStatus status = OS_STATUS_TASK_NOT_FOUND;
    (void)runtime;
    (void)_ctx;
    (void)_mem;
    ...
    m3ApiReturn(status == OS_STATUS_OK ? os_timer_get_id(timer) : 0U);
}

static m3ApiRawFunction(os_wasm_import_os_timer_delete)
{
    m3ApiReturnType(uint32_t);
    m3ApiGetArg(uint32_t, timer_id);
    OsTimerHandle timer = os_timer_find_by_id(timer_id);
    (void)runtime;
    (void)_ctx;
    (void)_mem;
    ...
    if (timer == NULL)
    {
        m3ApiReturn((uint32_t)OS_STATUS_TIMER_NOT_FOUND);
    }
...
static m3ApiRawFunction(os_wasm_import_os_timer_change_period)
{
    ...
    OsTimerHandle timer = os_timer_find_by_id(timer_id);
    OsStatus status = timer != NULL
        ? os_timer_change_period(timer, period_ms)
        : OS_STATUS_TIMER_NOT_FOUND;
    (void)runtime;
    (void)_ctx;
    (void)_mem;
...
static m3ApiRawFunction(os_wasm_import_os_timer_is_active)
{
    ...
    m3ApiReturn((uint32_t)os_timer_is_active(timer));
}
...
static OsStatus os_wasm_result_to_status(M3Result result)
{
    ...
    if (result == OS_WASM_LIBRARY_EXPORT_NOT_FOUND_ERROR ||
        strcmp(result, OS_WASM_LIBRARY_EXPORT_NOT_FOUND_ERROR) == 0)
    {
        return OS_STATUS_WASM_EXPORT_NOT_FOUND;
    }

    if (result == OS_WASM_LIBRARY_SIGNATURE_MISMATCH_ERROR ||
        strcmp(result, OS_WASM_LIBRARY_SIGNATURE_MISMATCH_ERROR) == 0)
    {
        return OS_STATUS_WASM_SIGNATURE_MISMATCH;
    }
...
static void os_process_timers(void)
{
    ...
    while (timer != NULL)
    {
        ...
        if (!os_task_is_in_list(timer->target_task) ||
            os_task_is_dead(timer->target_task))
        {
            os_timer_list_remove(timer);
            os_timer_free(timer);
            if (g_os.timer_count > 0U)
        ...
        if (timer->active &&
            os_time_reached(g_os.tick_ms, timer->expiry_tick_ms))
        {
            if (timer->auto_reload)
            {
                uint32_t elapsed_ms = g_os.tick_ms - timer->expiry_tick_ms;
                uint32_t elapsed_periods = elapsed_ms / timer->period_ms + 1U;
                timer->expiry_tick_ms += elapsed_periods * timer->period_ms;
            }
            else
            {
                timer->active = 0U;
...
static void os_update_time_events(void)
{
    g_os.processing_time_events = 1U;
    os_process_timers();
    os_update_waiting_tasks();
    g_os.processing_time_events = 0U;
    os_clock_rearm();
}
...
static void os_clock_rearm(void)
{
    uint32_t delay_ms = OS_WAIT_FOREVER;
    uint32_t target_tick_ms = 0U;

    if (g_os.clock_port.arm_wakeup == NULL ||
        g_os.clock_port.cancel_wakeup == NULL)
    {
        return;
    }

    if (g_os.processing_time_events)
    {
        return;
    }

    delay_ms = os_get_next_wakeup_ms();
    if (delay_ms == OS_WAIT_FOREVER)
    {
        if (g_os.clock_wakeup_armed)
        {
            g_os.clock_port.cancel_wakeup(g_os.clock_port.context);
            g_os.clock_wakeup_armed = 0U;
            g_os.clock_armed_tick_ms = 0U;
    ...
    target_tick_ms = g_os.tick_ms + delay_ms;
    if (g_os.clock_wakeup_armed &&
        g_os.clock_armed_tick_ms == target_tick_ms)
    {
        return;
    }
    ...
    g_os.clock_wakeup_armed = 1U;
    g_os.clock_armed_tick_ms = target_tick_ms;
    g_os.clock_port.arm_wakeup(g_os.clock_port.context, delay_ms);
}
...
static OsTaskRunResult os_run_task_slice(OsTaskHandle task)
{
    ...
    if (task->pending_wasm_library_call != NULL)
    {
        OsTaskRunResult library_result =
            os_run_wasm_library_call(task, &remaining_fuel);

        if (library_result != OS_TASK_RUN_LIBRARY_COMPLETE)
        {
            if (library_result == OS_TASK_RUN_FUEL_EXHAUSTED)
            {
                task->wasm_needs_resume = 1U;
            }
            else
        ...
        phase = "resume_after_wasm_library";
        m3_SetFuel(task->wasm_runtime, remaining_fuel);
...
static void os_request_task_stop(OsTaskHandle task)
{
    ...
    if (task->pending_wasm_library_call != NULL &&
        task->pending_wasm_library_call->instance != NULL &&
        task->pending_wasm_library_call->instance->active &&
        task->pending_wasm_library_call->instance->wasm_runtime != NULL)
    {
        m3_SetFuel(
            task->pending_wasm_library_call->instance->wasm_runtime,
            0U
        );
    }
}
